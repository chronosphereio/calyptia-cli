		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, ns := range namespaces.Items {
				...
				if err != nil {
					...
					fmt.Fprintf(cmd.OutOrStdout(), "a problem occurred while deleting deployments, err: %v\n", err)
				}
				...
				for _, item := range services.Items {
					...
					if err != nil {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "a problem occurred while deleting %s, err: %v\n", item.Name, err)
					}
				}
			...
			fmt.Fprintf(cmd.OutOrStdout(), "calyptia-core instance %q was successfully deleted", agg.Name)
			return nil
		},
	}
	fs := cmd.Flags()
