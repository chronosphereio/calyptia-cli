...
func NewPipelineModel(ctx context.Context, cloud client.Client, projectID, pipelineKey string, metricsStart, metricsInterval time.Duration) PipelineModel {
...
func (m PipelineModel) loadPipelineID() tea.Msg {
	...
	if len(aa.Items) != 1 && !cfg.ValidUUID(m.pipelineKey) {
		if len(aa.Items) != 0 {
