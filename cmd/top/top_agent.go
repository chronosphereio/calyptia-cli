...
func NewAgentModel(ctx context.Context, cloud client.Client, projectID, agentKey string, metricsStart, metricsInterval time.Duration) AgentModel {
...
func (m AgentModel) loadAgentID() tea.Msg {
	...
	if len(aa.Items) != 1 && !cfg.ValidUUID(m.agentKey) {
		if len(aa.Items) != 0 {
...
func agentMetricsToTableRows(metrics cloud.AgentMetrics) []table.Row {
	var rows []table.Row
	for _, measurementName := range metric.MeasurementNames(metrics.Measurements) {
...
func (row AgentMetricsRow) Render(w io.Writer, m table.Model, i int) {
	str := fmt.Sprintf("%s\t%s\t%s\t%s\t%s", row.PluginName, utils.ByteCell{Value: row.Rates.InputBytes}, utils.RecordCell{Value: row.Rates.InputRecords}, utils.ByteCell{Value: row.Rates.OutputBytes}, utils.RecordCell{Value: row.Rates.OutputRecords})
	if m.Cursor() == i {
