	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, k8s.ErrCoreOperatorNotFound) {
				return fmt.Errorf("core operator not found. Please install the core operator first")
			}
			if err != nil {
				return err
			}
			...
			syncDeployment, err := k8sClient.DeployCoreOperatorSync(ctx, operatorVersion, !noTLSVerify, created, serviceAccount.Name)
			if err != nil {
				fmt.Printf("An error occurred while creating the core operator instance. %w Rolling back created resources.\n", err)
				resources, err := k8sClient.DeleteResources(ctx, resourcesCreated)
				if err != nil {
