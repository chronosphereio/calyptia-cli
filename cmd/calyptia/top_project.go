...
func projectMetricsToTableRows(metrics cloud.ProjectMetrics) []table.Row {
	var rows []table.Row
	for _, measurementName := range projectMeasurementNames(metrics.Measurements) {
...
func (row ProjectMetricRow) Render(w io.Writer, m table.Model, i int) {
	fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%s\n", row.Plugin, ByteCell{row.Rates.InputBytes}, RecordCell{row.Rates.InputRecords}, ByteCell{row.Rates.OutputBytes}, RecordCell{row.Rates.OutputRecords})
}
...
func (row AgentTotalMetricsRow) Render(w io.Writer, m table.Model, i int) {
	str := fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s", row.Agent.Name, row.Agent.Type, row.Agent.Version, ByteCell{row.Rates.InputBytes}, RecordCell{row.Rates.InputRecords}, ByteCell{row.Rates.OutputBytes}, RecordCell{row.Rates.OutputRecords})
	if m.Cursor() == i {
